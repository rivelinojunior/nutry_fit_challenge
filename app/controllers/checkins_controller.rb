class CheckinsController < ApplicationController
  ...
  def create
    ...
    in Solid::Success[type: :created]
      redirect_to root_path, notice: "Check-in registrado."
    in Solid::Failure[type: :invalid_input]
      redirect_to root_path, alert: "Tarefa inválida."
    ...
    in Solid::Failure[type: :task_outside_challenge]
      redirect_to root_path, alert: "Tarefa não pertence a este desafio."
    in Solid::Failure[type: :not_scheduled_for_today]
      redirect_to root_path, alert: "Esta tarefa não é de hoje."
    in Solid::Failure[type: :outside_allowed_window]
      redirect_to root_path, alert: "Esta tarefa não está disponível agora."
    in Solid::Failure[type: :duplicate_checkin]
      redirect_to root_path, alert: "Você já marcou esta tarefa."
    in Solid::Failure[type: :validation_failed]
      redirect_to root_path, alert: "Não foi possível registrar o check-in."
    end
  end
end
